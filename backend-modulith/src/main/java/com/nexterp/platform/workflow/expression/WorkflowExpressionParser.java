...
public class WorkflowExpressionParser {
    ...
    private Object parseVariableExpression(String expression, ExpressionContext context) {
        ...
        if (expression.matches("^\\$\\{[^}]+}$")) {
            Pattern singleVarPattern = Pattern.compile("^\\$\\{([^}]+)}$");
            Matcher singleVarMatcher = singleVarPattern.matcher(expression);
            if (singleVarMatcher.matches()) {
                return resolveVariable(singleVarMatcher.group(1), context);
            }
        }
